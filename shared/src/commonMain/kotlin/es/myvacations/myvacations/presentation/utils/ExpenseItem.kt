...
import myvacations.shared.generated.resources.new_trip_optional_value_error
        ...
        AlertDialog(
            ...
            text = {
                Column {
                    ...
                    //TODO Agregar la divisa cuando este lista
                    Text(stringResource(Res.string.new_trip_optional_amount, "€"))
                    ...
                    OutlinedTextField(
                        value = onExpenseAmount.value.toString(), onValueChange = {
                            onExpenseAmount.value = it.toDoubleOrNull() ?: 0.0
                        }, keyboardOptions = KeyboardOptions(
                            keyboardType = KeyboardType.Decimal
                        ), singleLine = true
                    )
                    if (onExpenseAmount.value == 0.0) {
                        Text(
                            text = stringResource(Res.string.new_trip_optional_value_error),
                            color = MaterialTheme.colorScheme.error,
                            style = MaterialTheme.typography.bodySmall
                        )
                    }
            ...
            confirmButton = {
                TextButton(
                    onClick = {
                        if ((onExpenseName.value.isBlank() || onExpenseAmount.value == 0.0).not()) {
                            onUpdateExpenseName(expense.id, onExpenseName.value)
                            onUpdateExpenseAmount(expense.id, onExpenseAmount.value)
                ...
                Row {
                    ...
                    TextButton(
                        onClick = {
                            ...
                            onExpenseAmount.value = expense.amount
